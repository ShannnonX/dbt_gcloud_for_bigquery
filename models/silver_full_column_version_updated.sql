...
with clean_duplicate_row as (
    SELECT
        *
        ...
        FROM
            `house-prediction-381923.melbourne_house_data.bronze_full_column_version_updated` ) 
        WHERE
...
ON
  ...
  AND (
    PreAbsIndexPrice2.YearMonth = FORMAT_DATE('%Y-%m', DATE_SUB(PARSE_DATE('%Y-%m-%d', Bronze.Date), INTERVAL 3 MONTH))
    )
WHERE
